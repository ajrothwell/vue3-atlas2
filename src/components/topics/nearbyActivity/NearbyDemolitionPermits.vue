...
const nearbyDemolitionPermitsGeojson = computed(() => {
  let features = [];
  if (!nearbyDemolitionPermits.value) return features;
  for (let item of nearbyDemolitionPermits.value) {
    features.push({
      type: 'Feature',
      geometry: { type: 'Point', coordinates: [item.lng, item.lat] },
      properties: { id: item.objectid, type: 'nearbyDemolitionPermits' }
    })
  }
  return features;
})
watch (() => nearbyDemolitionPermitsGeojson.value, async (newGeojson) => {
  if (newGeojson.length > 0) {
    console.log('watch nearbyDemolitionPermitsGeojson, map.getStyle().sources:', map.getStyle().sources, 'map.getStyle().layers:', map.getStyle().layers);
...
onMounted(() => {
  console.log('NearbyDemolitionPermits.vue onMounted, nearbyDemolitionPermitsGeojson.value:', nearbyDemolitionPermitsGeojson.value);
  if (nearbyDemolitionPermitsGeojson.value.length > 0) {
    let geojson = { 'type': 'FeatureCollection', 'features': nearbyDemolitionPermitsGeojson.value }
    map.getSource('nearby').setData(geojson);
  }
});
